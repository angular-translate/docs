NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$emitted $on $rooscope $rootscope $scope $translate $translate- $translatechangesuccess $translateprovider aber add aendern aendert alle allen allerdings alles allgemeine als angular angular-translate angular-welt anotherone anstatt anwendungsf app app-titel arbeitet asynchron au auch auf aufgel augeloest aus ausgeworfen aussehen awesome beachte beachtet bedeteut bedeutet befinden beispiel benutzen benutzung beruecksichtigt bietet bitte code config controller controllers ctrl da dabei dar das dass data-binding deinem deinen deiner deines dem den der des die dies diese dieser dinge dir direkt direkte direktive direktiven dot-notation du durch eigentlich ein eine einem einer eines einfach einmal einzelne en entweder er erklaert es event expose falls features fehlerbehandlung filter function funktionierendes ganz gefeuert gekoppelt geladen geschehen gewechselt gibt guide headline heraus hier horcht html-dokuments immer informiert inhalte injecten innerhalb ins ist jedes jedoch jetzt kann kannst kein keine kommt konnten laden laufzeit level love machen mal man manipulieren mehrere mit module moeglich moelglich muessen muss musst myapp nachdenkst namespace namespaced_paragraph namespaces ng-app ng-controller nicht nichts niemals nur ob objekt oder overview paragraph pascalprecht performance-gruenden post preferredlanguage promise registriert rolle sch sehr sein seite service services sich sofern sogar sollten spiel spielt sprache sprachtabelle sprich srsly stark stelle stellte table tats translate translation translation-daten translationids translations tun two-way uebersetzt uebersetzung uebersetzungen um und unicorn uns unterstuetzt var verhaelt verschiedene von vor vorher warum weg wege weiter wenn werden wir wird wuerde wurde wurden zu zum zur zurueck"
    },
    {
      "section": "guide",
