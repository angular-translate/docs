NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$emitted $on $rootscope $scope $translate $translate- $translatechangesuccess $translateprovider aber add alle allen allerdings alles allgemeine als angular angular-translate angular-welt anotherone anstatt anwendungsf app app-titel arbeitet asynchron au auch auf aufgel aus ausgel ausgeworfen aussehen awesome beachte beachtet bedeteut bedeutet befinden beispiel benutzen benutzung ber bietet bitte code config controller controllers ctrl da dabei dar das dass data-binding deinem deinen deiner deines dem den der des die dies diese dieser dinge dir direkt direkte direktive direktiven dot-notation du durch ebenfalls eigentlich ein eine einem einer eines einfach einmal einzelne en entweder er erklaert es event expose falls features fehlerbehandlung filter folgt function funktionierendes ganz gefeuert gekoppelt geladen geschehen gewechselt gibt guide headline heraus hier horcht html-dokuments immer informiert inhalte injecten innerhalb ins ist jedes jedoch jetzt kann kannst kein keine kommt konnten laden laufzeit level love machen mal man manipulieren mehrere mit module muss musst myapp nachdenkst namespace namespaced_paragraph namespaces ng-app ng-controller nicht nichts niemals nur ob objekt oder overview paragraph pascalprecht performance-gruenden post preferredlanguage promise registriert rolle sch sehr sein seite service services sich sofern sogar sollten spiel spielt sprache sprachtabelle sprich srsly stark stelle stellte table tats translate translation translation-daten translationids translations tun two-way um und unicorn uns unterst var verh verschiedene von vor vorher warum weg wege weiter wenn werden wie wir wird wurde wurden zu zum zur"
    },
    {
      "section": "guide",
