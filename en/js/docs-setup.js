NG_DOCS={
  ...
  "pages": [
    ...
    },
    {
      ...
      "keywords": "$http $on $rootscope $scope $translate $translatepartialloader $translatepartialloaderprovider $translatepartialloaderstructurechanged $translateprovider $translationprovider _json_ accessing accross achieve activities add adding addition additional addpart adds advanced ago allgemeinen allright alright als amount anf angegeben angular angular-translate angular-translate-loader-partial angular-translate-loader-static-files angular-translate-loader-url angularjs app applied apps argument asynchronous asynchronously attribut attribute auch automate automatically awesome basically beautifully beginners behavior best better big bigger bin bind binding bit bootstraps bower break browser bug build button_lang_de button_lang_en cache caching called care case change changelanguage changes changing chapter charm checks chosen clever code combination comment common complete complex concatenate config configuration configure configures contact contactctrl content contents control controller controllers cool copy corresponding course covered ctrl current currently custom customloader data de dealing dedicated default demand den depends describes details deutsch devtools die dies differently disable docs document doesn don double drawback drops ein eine embedded embeded en enable endpoint englisch english ernsthaft erweiterte event everytime example execute executed executing existing expects extension extensions false file files fired fires fixed flash flickering foo foobar forward fouc function functionality german gilt gro guide ha hard haven headline hello hello_text hey html ich imagine inital inject install installable installed instance instances instantiated interesting internally interpolated interpoliert invoke invoked isn jedem json kann key kind konfiguration konstructor lang language languages launches lazy learn learned listening ll load loaded loader loaders loading loads locale locale- locale-de locale-en localization logical looked love main mainapp manipulate match method modul module modules myapp named needed network ng-click ng-controller ngcookies ngroute notice notices oder open options order org overridden overview package paragraph parameter partial partialloader parts pascalprecht passed passed_as_attribute passed_as_interpolation passed_as_text pattern plain post preferredlanguage prefix pretty process promise property provide provided provider pull quotes real refresh refreshing registered reloads request requests required respond return returns router runtime sample send separate server service services settinga setup short shortcut shortcut-konstruktoren simplest simply sits sowohl specifc specific specifies srsly standard startup static staticfilesloader straight string structured stuff submodule submodules suffix support supports table tables telling text things three time transform translate translate-values translating translation translations ui unicorn untranslated update url urlloader urltemplate useloader useloadercache user usestaticfilesloader useurlloader var variable_replacement ve wanna wasn werden wie won work working works world wurde yea yourspecialcacheservice"
    },
    {
      "section": "guide",
